...
export default function ContasPage() {
  ...
  const [editandoId, setEditandoId] = useState(null) // null para cadastro, id para edição
    ...
    <div className="space-y-8">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {contas.map((conta) => (
            <div key={conta.id_conta} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all group">
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                  ...
                  <button 
                    onClick={() => handleExcluir(conta.id_conta)}
              ...
              <div className="text-2xl font-black text-slate-900">
                R$ {parseFloat(conta.saldo_inicial).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
              </div>
            </div>
          ))}
          ...
          {contas.length === 0 && !loading && (
             <div className="col-span-full py-12 text-center bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                <p className="text-slate-400 font-bold">Nenhuma conta cadastrada ainda.</p>
             </div>
          )}
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[32px] p-8 shadow-2xl">
            <h2 className="text-2xl font-black text-slate-900 mb-6">
              {editandoId ? 'Editar Conta' : 'Cadastrar Conta'}
            </h2>
            ...
            <form onSubmit={handleSalvarConta} className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border-none rounded-2xl p-4 mt-1 focus:ring-2 focus:ring-indigo-500 outline-none font-medium text-slate-900"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-bold text-slate-400 ml-1 text-xs">Agência (Opcional)</label>
                  <input 
                    placeholder="0000"
                    className="w-full bg-slate-50 border-none rounded-2xl p-4 mt-1 focus:ring-2 focus:ring-indigo-500 outline-none font-medium text-slate-500"
                    value={agencia} onChange={(e) => setAgencia(e.target.value)}
                  />
                </div>
                <div>
                  <label className="text-sm font-bold text-slate-400 ml-1 text-xs">Nº Conta (Opcional)</label>
                  <input 
                    placeholder="00000-0"
                    className="w-full bg-slate-50 border-none rounded-2xl p-4 mt-1 focus:ring-2 focus:ring-indigo-500 outline-none font-medium text-slate-500"
              ...
              <div className="flex gap-3 mt-8">
                <button 
                  type="button" 
                  onClick={() => { setModalAberto(false); limparCampos(); }}
